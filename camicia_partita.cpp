//CAMICIA - Simulazione di una partita
//Stampa le carte in mano ai giocatori e le carte in gioco
...
//CIG è costruito in modo tale che le carte più recenti siano in fondo
//Quando CIG viene preso da un giocatore viene correttamente "capovolto"
//(nel gioco reale infatti le carte dei giocatori sono a faccia in giù, mentre CIG è a faccia in su)
...
char gioca(vector<int> G1, vector<int> G2) {
	...
	while(!controllaVuoto(G1) && !controllaVuoto(G2)) { //il gioco continua finché uno dei giocatori ha zero carte in mano
...
int main() {
	...
	cout << "Vincitore: G" << Winner << endl;
	cout << "Numero di carte giocate: " << nCarteGiocate << endl;
